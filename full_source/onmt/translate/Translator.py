...
def make_translator(opt, report_score=True, out_file=None):
    ...
    translator = Translator(model, fields, device=device, beam_size=opt.beam_size, global_scorer=scorer,
...
class Translator(object):
    ...
    def _run_target(self, batch, data, context, translate_part):
        ...
        if self.model.doc_context and translate_part in ["all", "context"]:
            if self.context_type == "HAN_join":
                memory_bank, attn_word_enc, attn_sent_enc = self.model.doc_context[0](src, memory_bank, memory_bank,
                                                                                      context)
            elif self.context_type == "HAN_enc" or self.context_type == "HAN_dec_source":
                memory_bank, attn_word_enc, attn_sent_enc = self.model.doc_context(src, memory_bank, memory_bank,
                                                                                   context)
        ...
        if self.model.doc_context and translate_part in ["all", "context"]:
            if self.context_type == "HAN_join":
                decoder_outputs, attn_word_dec, attn_sent_dec = self.model.doc_context[1](tgt_in, dec_out, dec_out,
                                                                                          context)
            elif "HAN_dec" in self.context_type:
